...
echo "Тип" > gothitelle1/totodileecho "===== Task 1 ====="
mkdir lab0
cd lab0
mkdir -p gothitelle1/{shellos,feebas}
mkdir -p klang9/{azurill,lopunny}
...
timeout 1 cat -n $(ls -1 lab0/tepig0/* 2>/dev/null | sort -r) 2>/dev/null
...
ls -lR lab0 2>&1 | grep -E '^-.*e$' | sort -k5,5nr | tail -n 3
...
ls -lR lab0 2>/dev/null | grep '^-.*to' | sort -k2,2nr | tail -n 2S
